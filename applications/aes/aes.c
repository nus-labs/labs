...
void finish(uint8_t *ciphertext, const uint8_t *const_cipher){
	// just want to load the ciphertext for the testbench to read the results
	uint8_t temp;
	for (int i = 0; i < AES_BLOCK_SIZE; i++) {
		temp = ciphertext[i] == const_cipher[i];
	}
}
